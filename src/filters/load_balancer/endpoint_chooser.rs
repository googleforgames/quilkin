...
impl EndpointChooser for RoundRobinEndpointChooser {
    fn choose_endpoints(&self, ctx: &mut ReadContext) {
        ...
        ctx.destinations = vec![ctx.endpoints[count % ctx.endpoints.len()].clone()];
    }
}
...
impl EndpointChooser for RandomEndpointChooser {
    fn choose_endpoints(&self, ctx: &mut ReadContext) {
        ...
        ctx.destinations = vec![ctx.endpoints[index].clone()];
    }
}
...
impl EndpointChooser for HashEndpointChooser {
    fn choose_endpoints(&self, ctx: &mut ReadContext) {
        ...
        ctx.destinations =
            vec![ctx.endpoints[hasher.finish() as usize % ctx.endpoints.len()].clone()];
    }
}
