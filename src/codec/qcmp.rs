...
/// `NonceBucket` keeps a pool of nonces that can be leased and ensures that a given nonce lease can
/// only be held by one leaser at a time
#[derive(Debug, Clone)]
struct NonceBucket {
...
struct NonceLease {
    bucket: Arc<std::sync::Mutex<Vec<u8>>>,
    nonce: u8,
}
...
impl Drop for NonceLease {
    fn drop(&mut self) {
        let mut guard = match self.bucket.lock() {
            Ok(guard) => guard,
            Err(poisoned) => {
                let guard = poisoned.into_inner();
                tracing::warn!("recovered from poisoned mutex");
...
mod tests {
    ...
    async fn nonce_bucket() {
        ...
        let nonce_bucket = NonceBucket::new();
        ...
        assert!(nonce_bucket.lease().is_none());
